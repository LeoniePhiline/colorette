...
export class Colorette {
  constructor({ useColor } = { useColor: isColorSupported }) {
    this.options = { useColor }

    Object.keys(colors).forEach((key) => {
      const color = colors[key]
      this[key] = (s) => (this.options.useColor ? color(s) : s)
    })
  }
}

export const createColors = (options = { useColor: isColorSupported }) => ({
  options,
  ...Object.entries(colors).reduce((colorMap, [key, color]) => ({
    ...colorMap,
    [key]: (s) => (options.useColor ? color(s) : s),
  })),
})
